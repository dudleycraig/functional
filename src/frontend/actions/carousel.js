...
export const fetchCarouselImage = (source, target, item) => {
  return dispatch => {
    ...
    return fetch(source, {method:'get'})
      .then(response => {
          if(!response.ok) {
            ...
            response.arrayBuffer().then(buffer => {
                const base64Image = ('data:image/jpeg;base64,' + window.btoa([].slice.call(new Uint8Array(buffer)).reduce((binary, b) => binary += String.fromCharCode(b), '')));
                return(base64Image);
            })
      ...
      .catch(error => dispatch(carouselImageErrorHandler(error, target)));
  }
}
